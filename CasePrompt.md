...
本文档汇总了项目中所有的案例提示词（prompt-1.md 至 prompt-10.md），按编号排列，供查阅和复用。
...
## 案例 6（prompt-6.md）：ESG、融资约束与盈利能力的回归分析
